"""
Secure Career Quick Coach Chatbot
"""
...
class SecureWCCChatbot:
    ...
    def process_message(self, user_message: str) -> Dict:
        ...
        if is_injection:
            ...
            return {
                'response': "I noticed your message might be trying to change how I work. I'm here to help with career-related questions only! What would you like to know?",
                'blocked': True,
                'security_events': security_events,
                'processing_steps': ['❌ Blocked at injection detection']
            }
        
        if SecurityGuardrails.detect_resume_writing_attempt(user_message):
            SecurityGuardrails.log_security_event(
                'RESUME_WRITING_ATTEMPT',
                f'Detected resume writing request',
                'WARNING'
            )
            security_events.append({
                'type': 'resume_writing_attempt'
            })
            ...
            return {
                ...
                'blocked': True,
                'security_events': security_events,
                'processing_steps': ['❌ Blocked at resume writing detection']
            }
        ...
        is_inappropriate, flagged_words, is_crisis, is_career_crisis = SecurityGuardrails.moderate_content(redacted_message)
        ...
        if is_inappropriate:
            ...
            return {
                'response': "I'm here to provide helpful career advice in a respectful manner. Please rephrase your question, and I'll be happy to assist!",
                'blocked': True,
                'security_events': security_events,
        ...
        if is_career_crisis:
            SecurityGuardrails.log_security_event('CAREER_CRISIS_DETECTED', 'Help needed', 'WARNING')
            return {
                'response': "I'm really concerned about what you're sharing. Career setbacks can feel overwhelming, but this sounds like more than job stress.",
                'blocked': True,
                'security_events': [{'type': 'crisis', 'severity': 'critical'}],
                'processing_steps': processing_steps + ['🚨 Crisis intervention triggered']
            }
