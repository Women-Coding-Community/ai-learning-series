...
def compare_all_patterns():
    ...
    query = "What do I need to learn to become a programmer?"
    print(f"\n🔍 Query: {query}\n")
    ...
    for pattern_type, pattern_name in patterns:
        ...
        # response = SecureWCCChatbot(pattern_type=pattern_type).process_message(query)['response']
...
def run_demo():
    ...
    print("\n" + "="*70)
    print("SECURE CAREER QUICK COACH CHATBOT")
    print("="*70)
    ...
    security_tests = [
        ("Normal query", "How do I become a programmer?"),
    ...
    while True:
        ...
        if user_input.lower() in ['quit', 'exit', 'q']:
            print("\nThank you for using SECURE CAREER QUICK COACH CHATBOT! 🎓")
            break
        
        result = bot.process_message(user_input)
        print(f"\nJordan: {result['response']}")
